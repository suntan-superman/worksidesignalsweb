 ...
 * 2. Then: node scripts/create-stripe-products.mjs
 */
...
try {
  ...
  console.log('   Run this first: firebase functions:config:get > .runtimeconfig.json\n');
}

if (!stripeKey) {
  console.error('❌ ERROR: Stripe secret key not found!');
  console.error('\nPlease run these commands first:');
  console.error('  cd C:\\Users\\sjroy\\Source\\Merxus\\web\\functions');
  console.error('  firebase functions:config:get > .runtimeconfig.json');
  console.error('  node scripts/create-stripe-products.mjs');
  process.exit(1);
}
...
async function setupAllProducts() {
  ...
  try {
    ...
    console.log('═══════════════════════════════════════════════════\n');
    console.log('🎉 Done! Send me these price IDs and I\'ll update the code!\n');
